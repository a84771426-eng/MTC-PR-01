...
if (cpfInput && cpfSubmit && consultaChat) {
  ...
  const appendQuickReplies = (options, onSelect) => {
    ...
    const choices = options
      .map(
        (option) =>
          `<button type="button" class="consulta-quick-reply" data-quick-reply="${option}">${option}</button>`
      )
      .join("");
  ...
  const flowAfterVehicleFound = async () => {
    await sendBotMessageWithTyping(
      `<strong>Veículo PLACA ${plateValue} Encontrado com sucesso em nosso sistema.</strong>`,
      1500
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (currentStep === "cpf") {
        ...
        await sendBotMessageWithTyping(
          "🟢 Informe a <strong>Placa</strong> do Veículo:<br><br>(Sem espaços, sem traços,<br><strong>EM MAIÚSCULO</strong>)<br><br>Ex: XJS2890",
          1000
        );
        currentStep = "plate";
        configureForPlate();
      ...
      if (currentStep === "plate") {
        ...
        await sendBotMessageWithTyping(`🟢 CONFIRMAR BUSCA PARA PLACA: <strong>${plateValue}</strong> ❔`, 900);
